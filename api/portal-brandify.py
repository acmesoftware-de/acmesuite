...
"""Post-process Redocly `build-docs` output into the ACMEsuite brand:
  1. inject the shared portal top-navigation (bar-mark + links) right after <body>,
...
Idempotent-ish: only acts when the markers are present. Run from repo root:
...
def brandify(path):
    ...
    if 'class="portal-nav"' not in s and "<body>" in s:
        s = s.replace("<body>", "<body>" + NAV, 1); changed += 1
